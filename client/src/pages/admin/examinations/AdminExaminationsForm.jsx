...
const AdminExaminationsForm = () => {
  ...
  const fetchExamination = useCallback(async () => {
    try {
      setLoading(true);
      const response = await apiService.get(`/admin/examinations/${id}`);
      ...
      setFormData({
        ...
        lastDateToApply: exam.lastDateToApply
          ? exam.lastDateToApply.split("T")[0]
          : "",
        feeStructure: {
          amount: exam.feeStructure?.amount || "",
          currency: exam.feeStructure?.currency || "PKR",
          lastDate: exam.feeStructure?.lastDate
            ? exam.feeStructure.lastDate.split("T")[0]
            : "",
        },
      });
    } catch (error) {
    ...
    try {
      ...
      const submitData = {
        ...
        feeStructure: {
          ...
          amount: formData.feeStructure.amount
            ? parseFloat(formData.feeStructure.amount)
            : undefined,
        },
      };

      if (isEditing) {
        await apiService.put(`/admin/examinations/${id}`, submitData);
        toast.success("Examination updated successfully");
      } else {
        await apiService.post("/admin/examinations", submitData);
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="exam@college.edu.pk"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="Examination Office"
                />
              </div>
            </div>
          </div>
...
export default AdminExaminationsForm;
